...
VIEWER = PACKAGE / 'v28_3d_cad_motion_viewer.html'
OUT = PACKAGE / 'v28_wsl_http_viewer_smoke.json'
SHOT = PACKAGE / 'v28_wsl_http_viewer_smoke.png'
SHOT_LATER = PACKAGE / 'v28_wsl_http_viewer_smoke_later.png'
...
def artifact_set_sha256(manifest: dict) -> str:
    h = hashlib.sha256()
    for item in sorted(manifest['parts'], key=lambda row: row['name']):
        for key in ('step', 'stl'):
            path = Path(item[key])
...
def main() -> int:
    ...
    result = {
        ...
        'input_artifact_set_sha256': artifact_set_sha256(manifest),
        'status': 'fail',
    ...
    try:
        ...
        checks = [
            ...
            {'name': 'viewer loaded V28 actual CAD meshes', 'status': 'pass' if 'Loaded V28 9 CAD mesh parts' in status_text else 'fail', 'status_text': status_text},
            {'name': 'viewer ratio readout names physical balance linkage', 'status': 'pass' if 'physical pallet-to-balance linkage' in ratio_text_initial else 'fail', 'ratio_text': ratio_text_initial},
            {'name': 'viewer preserves non-watch-grade limitation wording', 'status': 'pass' if 'non-regulating and demonstrative' in claim_note and 'not watch-grade or native CAD animation' in claim_note else 'fail', 'claim_note': claim_note},
        ...
        result['checks'] = [{'name': 'V28 WSL HTTP viewer smoke executed', 'status': 'fail', 'error': f'{type(exc).__name__}: {exc}'}]
    finally:
        server.terminate()
        try:
