...
VIEWER = PACKAGE / "v29_3d_cad_motion_viewer.html"
OUT = PACKAGE / "v29_wsl_http_viewer_smoke.json"
SHOT = PACKAGE / "v29_wsl_http_viewer_smoke.png"
SHOT_LATER = PACKAGE / "v29_wsl_http_viewer_smoke_later.png"
...
def main() -> int:
    ...
    try:
        ...
        checks = [
            ...
            {"name": "viewer loaded generated CAD meshes", "status": "pass" if "Loaded V29 13 CAD mesh parts" in status_text else "fail", "status_text": status_text},
            {"name": "viewer preserves fallback/non-watch-grade limitation wording", "status": "pass" if "fallback transform motion" in claim_note and "non-regulating" in claim_note else "fail"},
            {"name": "viewer advances while playing", "status": "pass" if state_later["thetaDeg"] > state_before["thetaDeg"] and ratio_text_later != ratio_text_initial else "fail"},
            ...
            {"name": "pose control moves to requested angle", "status": "pass" if abs(state_pose_90["thetaDeg"] - 90.0) < 0.2 else "fail"},
        ...
        result["checks"] = [{"name": "V29 WSL HTTP viewer smoke executed", "status": "fail", "error": f"{type(exc).__name__}: {exc}"}]
    finally:
        server.terminate()
        try:
