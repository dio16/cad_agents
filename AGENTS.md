...
- **ワークツリーは使用しない。** 全作業はメインブランチで直接実行する。
- `superpowers:using-git-worktrees` スキルは使用しない。
- `worktrees` スキルは無効化済み (`~/.config/opencode/skills/.worktrees.disabled/`)。
- 隔離が必要な場合は、`git stash` または `git branch` で対応する。
