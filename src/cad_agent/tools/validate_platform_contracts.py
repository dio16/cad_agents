...
def validate() -> tuple[int, dict[str, object]]:
    ...
    cli = read(Path("src/cad_agent/cli.py"))
    add(checks, "runner delegates to cad-agent", "uv run cad-agent" in runner)
    add(checks, "cli exposes validate-docs", "validate-docs" in cli)
