...
| `POST /v1/cad/jobs` | CAD 生成ジョブを起動（同期 simulation） | 同期 |
| `GET /v1/cad/jobs/{job_id}` | CAD ジョブ状態を取得（同期 simulation） | 同期 |
...
| `GET /v1/artifacts/{artifact_id}` | artifact index metadata を取得 | 同期 |
...
| Contract | 必須項目 | 用途 |
|---|---|---|
| DFM/AM Profile | `profile_id`, `process`, `min_wall_mm`, `min_hole_diameter_mm`, `max_bbox_mm`, `supported_materials`, `rule_ids` | Specification の `manufacturing_profile` を製造ルールへ解決する |
| Build Volume Envelope | `dfm.build_volume.max`, `specification.parameter_table.length`, `width`, `height`, `profile.max_bbox_mm` | 保守的な axis-aligned bbox check。`length` / `width` / `height` のいずれかが `max_bbox_mm` を超えると失敗扱い |
| Worker Probe | `worker`, `mode`, `executable`, `native_available`, `status` | FreeCAD/OCCT・Blender executable を probe し、存在する場合のみ `mode=native` を記録；不在時は `surrogate` |
